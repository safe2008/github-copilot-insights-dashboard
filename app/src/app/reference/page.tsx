...
const METRICS: MetricDef[] = [
  ...
  {
    name: "AI Credit Consumption (by user)",
    page: "AI Credits",
    chart: "KPI + Bar Chart",
    description: "Per-user AI credits consumed for the selected month — a usage-metrics signal (not a billed total). Powers the user, team, and organization filters.",
    calculation: "SUM(ai_credits_used) grouped by user over the month, from the Copilot Usage Metrics per-user field. Team/org filters resolve via dim_enterprise_team_member and dim_org.",
    source: "Database: fact_copilot_usage_daily.ai_credits_used (Copilot Usage Metrics API, 2026-06-19)",
  },
  {
    name: "User Breakdown Table (AI Credits)",
    page: "AI Credits",
    chart: "Sortable Table",
    description: "Per-user AI credits consumed with active-day context.",
    calculation: "Columns: User, AI Credits Used, Days Active. SUM(ai_credits_used) and COUNT(DISTINCT day) per user. Display names resolved from dim_user.",
    source: "Database: fact_copilot_usage_daily.ai_credits_used (Copilot Usage Metrics signal)",
  },
  {
    name: "Team Breakdown Table (AI Credits)",
    page: "AI Credits",
    chart: "Table",
    description: "AI credits consumed per enterprise team, attributed via the team roster.",
    calculation: "SUM(ai_credits_used) over members of each enterprise team (dim_enterprise_team_member) for the selected month.",
    source: "Database: fact_copilot_usage_daily + dim_enterprise_team_member",
  },
  ...
  // ── Users Page ──
  {
    ...
    calculation: "LEFT JOIN of usage data (fact_copilot_usage_daily aggregated per user) with live seat data from GitHub API. Columns: User, License, Status, Days Active, Interactions, Accept %, AI Credits, Agent, Chat, CLI, Last Active, Cost/mo. AI Credits = SUM(ai_credits_used) per user over the range.",
  ...
  // ── Enterprise Teams Page ──
  {
    ...
    calculation: "Direct listing from dim_enterprise_team and dim_enterprise_team_member tables. Member counts computed via COUNT aggregate. AI credits (28d) = SUM(ai_credits_used) over team members for the trailing 28 days.",
    source: "dim_enterprise_team, dim_enterprise_team_member, fact_copilot_usage_daily",
  ...
  },
  {
    ...
    description: "Average interactions, code generated/accepted, lines of code added, and AI credits consumed per engaged user in each adoption phase.",
    calculation: "SUM(metric) / COUNT(DISTINCT user_id) grouped by ai_adoption_phase. Avg AI Credits = SUM(ai_credits_used) / users per phase.",
    source: "fact_copilot_usage_daily (ai_adoption_phase, ai_credits_used)",
  },
  {
